...
def test_appdisplay():
    ...
    assert ab.client_id == "contXdisplay"
    ...
    assert ab.subscribe_cmnd_version_msg == b'contX/display/cmnd/version'
    assert ab.topic_cmnd_settext_msg     == b'contX/display/cmnd/settext'
    assert ab.topic_cmnd_fill_msg        == b'contX/display/cmnd/fill'
    assert ab.topic_cmnd_gettext_msg     == b'contX/display/cmnd/gettext'
    # replies
    assert ab.topic_text_msg             == b'contX/display/text'
...
def test_display_subscribe_cb(mock_machine,mock_umqtt,mock_network,mock_ssd1306):
    ...
    ab.mqtt_display_subscribe_cb(b"contX/display/cmnd/fill",b'0')
    mock_ssd1306.return_value.fill.assert_called_with(0)
    ab.mqtt_display_subscribe_cb(b"contX/display/cmnd/fill",b'1')
    mock_ssd1306.return_value.fill.assert_called_with(1)

    # test request set text
    mock_umqtt.reset_mock()
    mock_machine.reset_mock()
    ab.mqtt_display_subscribe_cb(b"contX/display/cmnd/settext",b"hello world,0,5")
...
def test_begin(mock_machine,mock_network,mock_umqtt):
    ...
    # checking subscribes
    subscribe_calls = [
                        call(b'contX/display/cmnd/version'),
                        call(b'contX/display/cmnd/repoversion'),
                        call(b'contX/display/cmnd/download'),
                        call(b'contX/display/cmnd/install'),
                        call(b'contX/display/cmnd/memfree'),
                        call(b'contX/display/cmnd/settext'),
                        call(b'contX/display/cmnd/gettext'),
                        call(b'contX/display/cmnd/fill')
                      ]
